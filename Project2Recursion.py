class Student:
    ...
    def add_courses(self, course_codes):
        ...
        for combination in class_combinations: #goes through all combos in class_combinations list of courses
            ...
            for i in range(len(combination)): 
                course_info = combination[i]       #formats courses into something more pretty.
    ...
    def recursive_course_compares(self, course_codes, index, combinations2, combinations1):
        ...
        if index == len(course_codes): #This is the base case that tells the func. when to stop.
            combinations1.append(list(combinations2)) #this will take all course combos from 'all combinations' and add them to a separate list 'current combination.
            return #this only happens when the index becomes = to the course codes list length.
                   #The course codes list is made from the user of all the courses under the course code they enter
        ...
        if course_code in self.courses_dict: #to the ones searched by the user   
            ...
            for chosen in available_classes: 
                ...
                #kind of backtracking. It removes the last added course before trying the next section of same course
                    combinations2.pop() 


    def time_conflict_loop(self, course_info, combinations2): #checks for conflicts within a set of courses.
    
        for existing_course_info in combinations2:   #for each course it goes through next time conflict func.
    ...
    def time_conflict(self, course_info_1, course_info_2): #this checks for conflicts between 2 courses.
        #takes indexes 1 and 2 which are days and times. Splits times into two separate numbers.
        days_1, time_range_1 = course_info_1[1], course_info_1[2].split("-") 
        if len(time_range_1) < 2:
            return False  

        start_time_1, end_time_1 = int(time_range_1[0]), int(time_range_1[1]) #formats times into ints so they can be compared.
        
        days_2, time_range_2 = course_info_2[1], course_info_2[2].split("-")
        if len(time_range_2) < 2:
            return False  

        start_time_2, end_time_2 = int(time_range_2[0]), int(time_range_2[1]) #same for second 
